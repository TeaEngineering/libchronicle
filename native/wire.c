...
void wire_parse(unsigned char *base, int lim, wirecallbacks_t *cbs) {
    ...
    while (p < base + lim) {
        ...
        case 0x00 ... 0x7F: // NUM
            ...
            if (cbs->field_uint64)
                cbs->field_uint64(field_name, field_name_sz, (uint64_t)control, cbs);
            break;
        case 0xB9: // EVENT_NAME
            ...
            p += ev_name_sz;
            if (cbs->event_name)
                cbs->event_name(ev_name, ev_name_sz, cbs);
            if (wire_trace)
                printf("Event name '%.*s'\n", ev_name_sz, ev_name);
            break;
        case 0x8F: // PADDING
        ...
        case 0x8D: // I64_ARRAY
            ...
            if (wire_trace)
                printf(" Field %.*s = [...] (I64_ARRAY) used %" PRIu64 "/%" PRIu64 "\n", field_name_sz, field_name,
                       jlong2, padding64);
            p += 16;
            if (cbs->ptr_uint64arr)
                cbs->ptr_uint64arr(field_name, field_name_sz, jlong2, padding64, p, cbs);
            p += 8 * padding64;
            break;
        case 0x90: // FLOAT32
            memcpy(&float32, p, sizeof(float32));
            if (wire_trace)
                printf(" Field %.*s = %f (FLOAT32)\n", field_name_sz, field_name, float32);
        ...
        case 0xA5: // INT16
            ...
            if (cbs->field_uint64)
                cbs->field_uint64(field_name, field_name_sz, (uint64_t)padding16, cbs);
            p += 2;
            break;
        case 0xA6: // INT32
            memcpy(&padding32, p, sizeof(padding32));
            ...
            if (cbs->field_uint64)
                cbs->field_uint64(field_name, field_name_sz, (uint64_t)padding32, cbs);
            p += 4;
            break;
        case 0xA7: // INT64
            memcpy(&padding64, p, sizeof(padding64));
            if (wire_trace)
                printf(" Field %.*s = %" PRIu64 " (uint64)\n", field_name_sz, field_name, padding64);
            if (cbs->ptr_uint64)
                cbs->ptr_uint64(ev_name, ev_name_sz, p, cbs);
            if (cbs->field_uint64)
                cbs->field_uint64(field_name, field_name_sz, padding64, cbs);
            p += 8;
            break;
        case 0xB6: // TYPE_PREFIX
            ...
            if (cbs->type_prefix)
                cbs->type_prefix(type_name, type_name_sz, cbs);
            break;
        ...
        case 0xE0 ... 0xFF: // Text field value
            ...
            if (cbs->field_char)
                cbs->field_char(field_name, field_name_sz, field_text, field_text_sz, cbs);
            p += field_text_sz;
            break;
...
wirepad_t *wirepad_init(int initial_sz) {
    ...
    if (wire_trace)
        printf("wire pad created at %p sz %d\n", pad->base, pad->sz);
    pad->pos = pad->base;
    pad->nest = 0;
    return pad;
}
...
void wirepad_extent(wirepad_t *pad, int sz) {
    ...
    if (wire_trace)
        printf(" wirepad_extent base=%p used=%d need=+%d remain=%d\n", pad->base, used, sz, remain);
    while (remain < 0) {
        pad->sz = pad->sz * 2;
        pad->base = realloc(pad->base, pad->sz);
...
void wirepad_text(wirepad_t *pad, char *text) {
    ...
    if (wire_trace)
        printf("wirepad_text pos=%p writing d=%d overhead=%d\n", pad->pos, d, overhead);
...
void wirepad_field(wirepad_t *pad, char *text) {
    ...
    int overhead = d < 0x1F ? 1 : 4;
    if (wire_trace)
        printf("wirepad_field pos=%p writing text=%s d=%d overhead=%d\n", pad->pos, text, d, overhead);
    wirepad_extent(pad, d + overhead);
    if (d < 0x1F) {
        pad->pos[0] = 0xC0 + d;
    } else {
...
void wirepad_uint64_aligned(wirepad_t *pad, uint64_t v) {
    if (wire_trace)
        printf("wirepad_uint64 pos=%p writing uint64=%" PRIu64 "\n", pad->pos, v);
...
void wirepad_varint(wirepad_t *pad, uint64_t v) {
    ...
    if (wire_trace)
        printf("wirepad_varint pos=%p writing varint=%" PRIu64 "\n", pad->pos, v);
...
void wirepad_event_name(wirepad_t *pad, char *event_name) {
    ...
    if (wire_trace)
        printf("wirepad_event_name pos=%p name=%s\n", pad->pos, event_name);
    if (d > 0xFF) {
...
void wirepad_type_prefix(wirepad_t *pad, char *type_prefix) {
    ...
    if (wire_trace)
        printf("wirepad_type_prefix pos=%p name=%s\n", pad->pos, type_prefix);
    if (d > 0xFF) {
...
void           wirepad_qc_start(wirepad_t *pad, int metadata) {
    ...
    if (wire_trace)
        printf("wirepad_qc_start pos=%p meta=%d nest=%d header=0x%x\n", pad->pos, metadata, pad->nest, header);
    wirepad_extent(pad, 9);
...
void wirepad_qc_finish(wirepad_t *pad) {
    ...
    if (wire_trace)
        printf("wirepad_qc_finish pos=%p entered=%p len=%d (0x%x) nest=%d\n", pad->pos, entered, len, len, pad->nest);
...
void wirepad_nest_enter(wirepad_t *pad) {
    if (wire_trace)
        printf("wirepad_nest_start pos=%p nest=%d\n", pad->pos, pad->nest);
...
void wirepad_nest_exit(wirepad_t *pad) {
    ...
    int            len = pad->pos - entered - 4;
    if (wire_trace)
        printf("wirepad_nest_exit pos=%p entered=%p len=%d (0x%x) nest=%d\n", pad->pos, entered, len, len, pad->nest);

    uint32_t header = len;
    memcpy(entered, &header, sizeof(header));
}

void wirepad_parse(wirepad_t *pad, wirecallbacks_t *cbs) { wire_parse(pad->base, pad->pos - pad->base, cbs); }
...
void wirepad_write(unsigned char *base, void *msg, size_t sz) {
    wirepad_t *pad = (wirepad_t *)msg;
