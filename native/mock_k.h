...
K dot(K x, K y) { // call function pointer in x with args in mixed list y
    if (x->t != 100)
        return krr("x must be fptr");
    if (y->t != 0)
        return krr("y must be list");
    if (x->a == 2) {
...
K   ktn(int type, long long n) { // K type number
    ...
    r->t = type;
    if (n > 0)
        r->n = n; // keep: trap accessing n for atom in valgrind
    return r;
}
...
void r0(K x) { // Decrement the object‘s reference count
    if (x == 0) {
        printf("Bug r0 of null pointer %p\n", x);
        return;
    }
    if (x->r < 0)
        printf("Bug double-free of %p\n", x);
    if (x->r == 0) {
        if (x->t == 0)
            for (int i = 0; i < x->n; i++)
                r0(kK(x)[i]);
