...
//
// This file builds a shared-object "shmipc.so" for easy integration of
...
// Usual KDB indirection via arrays, since
// returning a void* to the runtime isn't an option, and there's
// no nice way to store it into a K object directly. 
...
K shmipc_init(K dir, K parser, K ver, K create) {
    if (dir->t != -KS) return krr("dir is not symbol");
    if (dir->s[0] != ':') return krr("dir is not symbol handle (starts with :)");
    if (ver->t != -KJ) return krr("ver is not j");
    if (create->t != -KB) return krr("create is not bool");
    ...
    chronicle_set_version(queue, ver->i);
    chronicle_set_create(queue, create->i);
